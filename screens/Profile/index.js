...
const Profile = (props) => {
  ...
  const zodiacEmoji = getZodiacSign(
  ...
  const friends = useSelector((state) => state.userState.friends).map(
    (item) => item?.uid
  );
  const studyBuddies = useSelector((state) => state.userState.studyBuddies).map(
    (item) => item?.uid
  );
  ...
  const burningQuestions = useBurningQuestions().burningQuestions.filter(
    (item) => item.uid == uid && !item.isAnonymous
  );
  ...
  const data = [
    ...
    },
    {
      ...
      onPress: () =>
        props.navigation.navigate("GraduationYearSettings", {
          gradYear: user?.gradYear,
        }),
      subtitle: getSubtitle(
        isCurrentUser() ? "graduate in" : "graduates in",
        user?.gradYear,
        false,
        false
      ),
      data: user?.gradYear ? user.gradYear : isCurrentUser() ? "Edit" : null,
    },
    {
      id: "2",
      emoji: "💯",
      title: "GPA",
      onPress: () =>
        props.navigation.navigate("GPASettings", { gpa: user?.gpa }),
      data: user?.gpa ? user.gpa : isCurrentUser() ? "Edit" : null,
    ...
    },
    {
      ...
      title: "Birthday",
      onPress: () =>
        props.navigation.navigate("BirthdaySettings", {
  ...
  const profileItemModal = (emoji, title, subtitle) => (
    ...
    >
      ...
      <View>
        <BoldText h4 tint style={{ marginTop: 20 }}>
          {title}
        </BoldText>
      </View>
      <View>
        <RegularText darkgray style={{ textAlign: "center", marginTop: 10 }}>
          {subtitle}
        </RegularText>
      </View>
      ...
      >
        <View
          style={[
            styles.profileItemCloseBtn,
            { backgroundColor: Colors[colorScheme].lightGray },
          ]}
        >
          <MediumText h4 accent>
            {"Close"}
          </MediumText>
        </View>
      </TouchableWithoutFeedback>
  ...
  const onAddFriendPress = () => {
    ...
    if (!isFriend()) {
      ...
      unAddFriend(uid).catch((e) => props.onTaskError(e.message));
    }
  };
  ...
  const onChatPress = () => {
    ...
    } else props.navigation.navigate("Chat", chat);
  };
  ...
  const HeaderRight = () => {
    return (
      <TouchableWithoutFeedback onPress={() => setShowModal(true)}>
        <CustomImage
          source={assets.more}
          style={{ width: 25, height: 25, tintColor: Colors[colorScheme].tint }}
        />
      </TouchableWithoutFeedback>
    );
  };
  ...
  const onProfileButtonPress = () => {
    if (isCurrentUser()) setShowImageOptionModal(true);
    else
      props.navigation.navigate("FullScreenMedia", {
        media: user.photoURL ? user.photoURL : getDefaultImage("private"),
      });
  };
    ...
    <View style={{ flex: 1, backgroundColor: Colors[colorScheme].background }}>
      ...
      >
        <OptionsList
          ...
          onOptionPress={[
            () => openMediaLibrary(setProfileImage),
            () => onTakePhotoPress(setProfileImage),
          ]}
        />
      </SlideModal>
      ...
      >
        <OptionsList
          ...
          options={
            isCurrentUser()
              ? ["Settings"]
              : [!isFriend() ? "Add Friend" : "Remove Friend", "Report"]
          }
          onOptionPress={
            isCurrentUser()
              ? [onSettingsPress]
              : [onAddFriendPress, onReportPress]
          }
        />
      </SlideModal>
      ...
      <ImageHeaderScrollView
        ...
        renderTouchableFixedForeground={() =>
          isCurrentUser() && (
            ...
            >
              <TouchableWithoutFeedback
                onPress={() => onTakePhotoPress(setHeaderImage)}
              >
                <View
                  style={{
                    marginRight: 10,
        ...
        renderHeader={() => {
          return user?.headerImage ? (
            <View style={{ width, height: MAX_HEIGHT }}>
              <CustomImage
      ...
      >
        <TriggeringView style={{ paddingHorizontal: 15 }}>
            ...
            <View
              style={[
                styles.calloutContainer,
                {
                  ...
                  top: 10,
                  right: 15,
                  backgroundColor: Colors[colorScheme].veryDarkGray,
                },
              ]}
            >
              <RegularText white>
                {"Add " + user.displayName + " as a Study Buddy!"}
              </RegularText>
              <View
                style={{
                  position: "absolute",
          ...
          <View
            style={[
              styles.profileHeaderContentContainer,
              {
                marginTop: isFriend() && !isStudyBuddy() ? 60 : 20,
                backgroundColor: Colors[colorScheme].invertedTint,
                ...SHADOWS[colorScheme],
              },
            ]}
          >
            {isFriend() && !isCurrentUser() && (
              <TouchableOpacity
                ...
                style={[
                  styles.studyBuddyContainer,
                  {
                    backgroundColor: isStudyBuddy()
                      ? Colors.accent
                  ...
                  {loadingStudyBuddy && (
                    <ActivityIndicator
                      color={
                        isStudyBuddy()
                          ? Colors.white
                          : Colors[colorScheme].veryDarkGray
                      }
                    />
                  )}
                  {!loadingStudyBuddy && (
                ...
                >
                  ...
                  >
                    <ProfileButton
                      onLongPress={() =>
                        props.navigation.navigate("FullScreenMedia", {
                          media: user.photoURL || getDefaultImage("private"),
                        })
                      }
                ...
                item.data && (
                  <TouchableOpacity
                    onPress={() =>
                      item.data == "Edit"
                        ? item.onPress()
                        : onProfileItemPress(item)
                    }
                    style={[
                      styles.profileItemContainer,
                      { borderColor: Colors[colorScheme].lightGray },
                    ]}
                  >
                    <LightText lightgray h5 style={{ textAlign: "center" }}>
                      {item.emoji + " " + item.data}
                    </LightText>

                    <TouchableWithoutFeedback onPress={item.onPress}>
                      <CustomImage
                        source={assets.pencil_o}
                        style={{
                          marginLeft: 8,
                          width: 15,
                          height: 15,
                          tintColor: Colors[colorScheme].veryDarkGray,
                        }}
                      />
                    </TouchableWithoutFeedback>
                  </TouchableOpacity>
          ...
          >
            <ProfileItemsButton
              ...
              subtitle={getProfileItemsSubtitle(
                burningQuestions,
                "Burning Question"
              )}

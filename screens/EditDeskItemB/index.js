import { View, ScrollView, Animated, FlatList, TouchableOpacity, TouchableWithoutFeedback } from 'react-native'
...
const EditDeskItemB = (props) => {
    ...
    const [showInExplore, setShowInExplore] = useState(deskItem?.showInExplore || false);
    const data = {
        classId,
        title: title.trim(),
    ...
    const uploadQuestions = (id) => new Promise((resolve, reject) => {
        for (let i = 0; i < questions.length; i++) {
            ...
            for (let j = 0; j < questions[i].answerChoices.length; j++) {
                if (questions[i].answerChoices[j].isImage) {
                    ...
                    const filename = media.substring(media.lastIndexOf('/') + 1);
                    const path = `desk-item/${auth.currentUser.uid}/${id + i + j}/${filename}`;
            ...
            postDeskItem(data)
                .then((id) => {
                    ...
                    if (type == "Flashcards")
                        ...
                        uploadQuestions(id)
                            .then(() => updateDeskItem(id, { questions }))
            ...
            updateDeskItem(deskItem.id, data)
                .then(() => {
                    if (type == "Flashcards")
                        ...
                        uploadQuestions(deskItemid)
                            .then(() => updateDeskItem(deskItem.id, { questions }))
    ...
    return (
        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1, backgroundColor: Colors[colorScheme].background }}>
                ...
                <CustomBottomSheet
                    height={350}
                    onToggle={() => setShowSubjectModal(false)}
                    show={showSubjectModal}
                    renderContent={() => <View style={{ padding: 15, alignItems: 'center' }}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity onPress={() => {
                                    ...
                                    <View style={{ backgroundColor: Colors[colorScheme].lightGray, borderRadius: 25, paddingHorizontal: 15, padding: 8, marginRight: 10, marginBottom: 10 }}>
                ...
                <ScrollView
                    ...
                    <StyledTextInput
                        ...
                        rightIcon={<CustomImage source={assets.down_arrow} style={{ width: 25, height: 25, tintColor: Colors[colorScheme].darkGray }} />}
                        isClearable={false}
                        editable={false}
                        value={subject}
                    ...
                    <LightText darkgray style={{ marginTop: 10 }}>{"Save your " + type + " anonomously to keep your name and profile picture from appearing on them. This will also hide them from others in your Desk."}</LightText>

                    <TouchableWithoutFeedback onPress={() => setShowInExplore(!showInExplore)}>


                        <View style={{ marginTop: 20, flexDirection: 'row', borderRadius: 50, padding: 10, justifyContent: 'space-between', backgroundColor: Colors[colorScheme].lightGray }}>
                    ...
                    <LightText darkgray>{type[type.length - 1] == 's' ?
                        'Post these ' + type + ' to Explore where anyone can view them, including students who are not in your school.' :
                        'Post this ' + type + ' to Explore where anyone can view it, including students who are not in your school.'}
                    </LightText>
